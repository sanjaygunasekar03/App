...
const App = () => {
  ...
  const handleAddBatch = (status) => {
    const newBatch = {
      id: `B-${newBatchId}`,
      title: status === 'Draft' ? `Draft Batch ${newBatchId}` : 
            status === 'In Queue' ? `Queue Batch ${newBatchId}` : 
            status === 'Review' ? `Review Batch ${newBatchId}` : 
            `Completed Batch ${newBatchId}`,
      date: new Date().toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }),
      calls: '0 / 0',
      info: '0',
      status: status,
        ...
        {batchStatuses.map((section, index) => (
          <div key={index} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="bg-gray-900 text-white p-3 flex justify-between items-center">
              ...
              <button className="text-sm text-blue-300 hover:text-blue-200">View all ({section === 'Review' ? batches.filter(b => b.status === 'Review').length : 0})</button>
            </div>
                ...
                <div className="space-y-3">
                  ...
                  {batches.filter(b => b.status === 'In Queue').length > 0 ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center p-4">
                      ...
                      <button 
                        onClick={() => handleAddBatch('In Queue')}
